...
def carve(file_path, output_dir, offset, size, magic=None):
    r2 = r2pipe.open(file_path, ['-z'])
    file_info = r2.cmdj('ij')
    if 'MDMP' not in file_info['core']['type']:
        print("[x] File type is not MDMP!")
        return None
    if magic:
    ...
    print("[+] Carving to %s.%s" % (output_dir, offset))
    return '%s.%s' % (output_dir, offset)
