...
import requests
from snake import config, db, error, fields, scale
...
class Interface(scale.Interface):
    def _vt_scan(self, sha256_digest, cache=True):
        params = {"apikey": API_KEY, "resource": sha256_digest, "allinfo": 1}
        ...
        if "vt" not in document or not cache:
            ...
            if not document or "vt" not in document:
                raise error.MongoError(
                    "error adding vt into file document %s" % sha256_digest
                )
        if document["vt"]["response_code"] == 0:
        ...
        if IS_PRIVATE and "first_seen" not in document["vt"]:
            raise error.InterfaceWarning(
                "private key specified but no private api data in cache, please flush vt cache for sample"
            )
    ...
    @scale.pull(
        {
            "args": {"cache": fields.Bool(missing=True)},
